...
def diverse_beam_search(model, tokenizer, prompt, beam_width=6, num_groups=3, max_length=50, device='cpu', diversity_penalty=1.0, epsilon=1e-12):
    input_ids = tokenizer(prompt, return_tensors="pt").input_ids.to(device)
    group_size = beam_width // num_groups
    beams = [[(input_ids, 0.0)] for _ in range(num_groups)] # Each group has its own beam
        ...
        for g in range(num_groups):
            ...
            for seq, score in beams[g]:
                ...
                for token in prev_tokens_counts:
                    ...
                    penalized_probs[0, token] /= (1.0 + diversity_penalty * prev_tokens_counts[token])
                ...
                for i in range(group_size):
                    ...
                    next_score = score + torch.log(topk_probs[0, i] + epsilon).item()
